...
__global__ void kernel_rows(const double *filter, const double *input, double *output,
  ...
  for(k = -filterR; k <= filterR; k++){
    ...
    sum += input_local[threadIdx.y * (2 * padding + blockDim.x) + padding + threadIdx.x + k] * filter[filterR - k];
...
__global__ void kernel_columns(const double *filter, const double *buffer, double *output,
  ...
  for(k = -filterR; k <= filterR; k++){
    ...
    sum += buffer_local[(threadIdx.y + padding + k) * blockDim.x + threadIdx.x] * filter[filterR - k];
...
int main(void) {
    ...
    *h_OutputCPU;
    ...
    // printf("Input[0][0] = %lf\n", h_Input[filter_radius * padding_imageH + filter_radius]);
    ...
    // printf("CPU Buffer:\n");
    // for(i = 0; i < padding_imageH; i++){
    //   for(j = 0; j < padding_imageH; j++)
    //     printf("%lf\t", h_Buffer[i * padding_imageW + j]);
    //   printf("\n");
    // }
    //////////////////////////////// GPU ///////////////////////////////////////
    dim3 block_dim;
    ...
    // cudaMemcpy(h_Buffer, d_Buffer, padding_imageW * padding_imageH * sizeof(double), cudaMemcpyDeviceToHost);
    // printf("GPU Buffer:\n");
    // for(i = 0; i < padding_imageH; i++){
    //   for(j = 0; j < padding_imageH; j++)
    //   printf("%lf\t", h_Buffer[i * padding_imageW + j]);
    ...
    // for(i = 0; i < padding_imageH; i++){
    //   for(j = 0; j < padding_imageH; j++)
    //   printf("%lf\t", h_OutputCPU[i * padding_imageW + j]);
    // printf("\n");
    // }
    // printf("GPU Output:\n");
    // for(i = 0; i < padding_imageH; i++){
    //   for(j = 0; j < padding_imageH; j++)
    //   printf("%lf\t", h_OutputGPU[i * padding_imageW + j]);
    // printf("\n");
    // }
    //////////////////////// RESULT COMPARISON /////////////////////////////////
