...
////////////////////////////////////////////////////////////////////////////////
__global__ void kernel_rows(const double *input, double *output,
  ...
  extern __shared__ filterStruct input_local[];

  for(k = - filterR; k <= filterR; k+=filterStep)
    input_local[threadIdx.y  * (2 * padding + blockDim.x) + threadIdx.x + padding + k].d = input[(idx_y + padding) * imageW + idx_x + padding + k];
  ...
  for(k = -filterR; k <= filterR; k++){
    ...
    sum += input_local[threadIdx.y * (2 * padding + blockDim.x) + padding + threadIdx.x + k].d * filter_constant[filterR - k].d;
...
////////////////////////////////////////////////////////////////////////////////
__global__ void kernel_columns(const double *buffer, double *output,
                       int imageW, int imageH, int filterR, int filterStep){
  int idx_x = threadIdx.x + blockDim.x * blockIdx.x;
  int idx_y = threadIdx.y + blockDim.y * blockIdx.y;
  ...
  extern __shared__ filterStruct buffer_local[];

  for(k = - filterR; k <= filterR; k+=filterStep)
    buffer_local[(threadIdx.y + padding + k)  * blockDim.x + threadIdx.x].d = buffer[(idx_y + padding + k) * imageW + idx_x + padding];
  ...
  for(k = -filterR; k <= filterR; k++){
    ...
    sum += buffer_local[(threadIdx.y + padding + k) * blockDim.x + threadIdx.x].d * filter_constant[filterR - k].d;
...
int main(void) {
    ...
    *h_OutputCPU;
    ...
    kernel_rows<<<grid_dim, block_dim, block_dim.y * (2 * filter_radius + block_dim.x) * sizeof(filterStruct)>>>(d_Input, d_Buffer, padding_imageW, padding_imageH, filter_radius, filterStep);
    cudaDeviceSynchronize();
    cudaCheckForErrors();

    // kernel_columns<<<grid_dim, block_dim>>>(d_Filter, d_Buffer, d_OutputGPU, padding_imageW, padding_imageH, filter_radius);
    kernel_columns<<<grid_dim, block_dim, block_dim.x * (2 * filter_radius + block_dim.y) * sizeof(filterStruct)>>>(d_Buffer, d_OutputGPU, padding_imageW, padding_imageH, filter_radius, filterStep);
    cudaDeviceSynchronize();
    cudaCheckForErrors();
